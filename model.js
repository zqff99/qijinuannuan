...
var category = ['发型','连衣裙','上装','下装','外套','袜子-袜套','袜子-袜子','鞋子','妆容','萤光之灵','饰品-头饰·发饰','饰品-头饰·头纱','饰品-头饰·发卡','饰品-头饰·耳朵','饰品-耳饰','饰品-颈饰·围巾','饰品-颈饰·项链','饰品-手饰·右','饰品-手饰·左','饰品-手饰·双','饰品-手持·右','饰品-手持·左','饰品-手持·双','饰品-腰饰','饰品-特殊·面饰','饰品-特殊·胸饰','饰品-特殊·纹身','饰品-特殊·翅膀','饰品-特殊·尾巴','饰品-特殊·前景','饰品-特殊·后景','饰品-特殊·顶饰','饰品-特殊·地面','饰品-皮肤'];
...
Clothes = function(csv) {
  ...
  return {
    ...
    tags: csv[14].split('/'),
    source: csv[15].replace(/设·图/g,"设计图").replace(/设·/g,"").replace(/抽·/g,""),
    isSuit: csv[16],
    version: csv[17],
    ...
    addDep: function(sourceType, depNum, c) {
		var depinfo = {};
		depinfo.sourceType = sourceType;//记录服装的来源
		depinfo.depNum = depNum;//记录服装要求的总数量
      if (c == this) {
        alert("Self reference: " + this.type.type + " " + this.id + " " + this.name);
      }
		depinfo.c = c;//需要制作的衣服对象
    ...
    getDeps: function(indent, parentDepNum) {
      var ret = '';
        for (var i in this.deps) {
          ...
          ret += indent + '[' + depinfo.sourceType + '][' + c.type.mainType + c.id + ']'
              + c.name + ((depNumAll == 0)? '' : '[需' + (depNumAll)  + ']') + ((c.own)? '[已有]' : '') + '\n';
          ret += c.getDeps(indent + "   ", depNumAll);
        }
		var depNumAlls = 0;var depNotNumAlls = 0;
		var splits=ret.split('\n');
		for (var i = 0; i<splits.length; i++){
			var depNums = splits[i].indexOf('需') < 0 ? 0 : splits[i].replace(/[^(需)]*需([0-9]+)[^0-9]*/,"$1");
			if (depNums) depNumAlls += Number(depNums);
			if(splits[i].indexOf('已有')==-1)  depNotNumAlls += Number(depNums);
		}
		
		if(indent == '   ' && ret != ''){
			ret = "[材料]" + this.name + (depNumAlls>0 ? (' - 总计需 '+ depNumAlls + ' 件 还差' + depNotNumAlls + "件") : "") + "\n" + ret;
...
function loadFromStorage() {
  ...
  //console.log(myClothesNew);
  //console.log("测试"+getCookie("mine"));
  if (myClothesNew) {
...
function CopyAllClothes(){
	myClothes.focus();
	myClothes.select();
	document.execCommand("Copy");
	document.getElementById("importCate").focus();
}
