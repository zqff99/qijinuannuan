...
function drawLevelInfo() {
	...
	if (currentLevel) {
		...
		if (currentLevel.styles) {
			var $fengge,
			$shuoming,
			fenggeStyle,
			shuomingStyle;
			if (currentLevel.styles[0]) {
				$fengge = $("<font>").text("风格:  ").addClass("shaonvSkill");
				fenggeStyle = "";
				fenggeStyle += currentLevel.styles[0] + "  ";
			}
			if (currentLevel.styles[1]) {
				$shuoming = $("<font>").text("说明:  ").addClass("hintInfo");
				shuomingStyle = "";
				shuomingStyle += currentLevel.styles[1] + "  ";
			}
			$styleZ.append($fengge).append(fenggeStyle)
			.append($shuoming).append(shuomingStyle);
		}
		if ($("#theme").val().indexOf('关卡: ')>=0) {
			var $itemName,$itemName2,
			itemInfo,itemInfo2,
			clothesCount,clothesCountText,clothesNums,zhuDiv;
			itemInfo = "";clothesNums=[-1,-1,-1,-1,-1];
			var guanka=$("#theme").val().replace("关卡: ","").replace(/(\d+?-.*?)-.*/g,"$1");
			var sn = 0;var gz = 3;
			for (var i in clothes){
				if(clothes[i].source.indexOf(guanka)>=0) {
					var temps = clothes[i].source.split("/");
					for(var j in temps){
						if(temps[j]==guanka+"少"){
							clothesNums[sn]=i;
							sn++;
			...
			var tempobj,temptextObj;
			for(var i=0;i<5;i++){
				zhuDiv=$("<div>").addClass("itemInfoShow");
				if(clothesNums[i]!=-1){//如果存在衣服编号
					if(i==0){
						$itemName = $("<font>").text("少女级掉落").addClass("itemInfo itemInfoShow");
					}else if(i == 3){
						$itemName = $("<font>").text("公主级掉落").addClass("itemInfo2 itemInfoShow");
					}else{
						$itemName = $("<font>").text("　　　　　").addClass("itemInfo itemInfoShow");
					}
					clothesCount=1;
					clothesCountText=clothes[clothesNums[i]].getDeps('   ',1);
					if(clothesCountText!=""){
						clothesCount=clothesCountText.match(/还差\d+/g)[0].replace("还差","");
					}
					itemInfo = clothes[clothesNums[i]].id + clothes[clothesNums[i]].type.mainType + "：" + (clothesCount==0 ? "1" : clothesCount) + "个 [";
					itemInfo += clothes[clothesNums[i]].source + "]　";
					tempobj=clothesNameTd(clothes[clothesNums[i]]).addClass("itemInfoShow");
					temptextObj=$("<span>").text(itemInfo).addClass("itemInfoShow");
					zhuDiv.append($itemName).append(tempobj).append(temptextObj);
